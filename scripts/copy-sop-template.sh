...
#   - 复制 CLAUDE.md 开发规范模板
#   - 复制 .claude/ Claude 配置 (CLAUDE.md, agents, rules, skills)
#   - 复制 Architect/ 架构模板指引
#   - 复制 Design/ 设计稿目录模板
#   - 复制 PRD/ 产品需求目录模板
...
echo ""
echo -e "${CYAN}[Claude Agents 配置]${NC}"
copy_dir "$SOURCE_DIR/.claude/agents" "$TEMPLATE_DIR/.claude/agents" ".claude/agents/ (代理配置)"

echo ""
echo -e "${CYAN}[Claude Rules 规则]${NC}"
copy_dir "$SOURCE_DIR/.claude/rules" "$TEMPLATE_DIR/.claude/rules" ".claude/rules/ (项目规则)"

echo ""
echo -e "${CYAN}[Claude Skills 技能]${NC}"
copy_dir "$SOURCE_DIR/.claude/skills" "$TEMPLATE_DIR/.claude/skills" ".claude/skills/ (技能配置)"
...
> **应用源码位于上一级目录 `../`，Claude 在分析代码时请从 `../` 开始检索。**
'
