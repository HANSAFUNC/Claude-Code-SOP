# 开发流程

**版本:** 3.3
**日期:** 2026-04-14
**来源:** Superpowers Pipeline v6.1

---

## 1. 流程概览

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Superpowers 三阶段硬门禁开发流程                           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Phase 1: Design      HARD-GATE 1      Phase 2: Planning                   │
│  /superpowers     ─────────────────→   /writing-plans                      │
│       ↓            "禁止未批准实施"          实施计划文档                     │
│  设计规格文档                              ↓                               │
│                                        HARD-GATE 2                         │
│                                    "计划必须可构建"                          │
│                                             ↓                               │
│  Phase 3: Implement  ─────────────→   HARD-GATE 3                          │
│  /subagent-driven       ↓             "规格合规 + 代码质量"                 │
│  /tdd /debug /verify  Git Worktree                                        │
│       ↓             隔离机制                                               │
│  双阶段审查                                                                 │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 核心原则

| 原则 | 说明 |
|------|------|
| **硬门禁** | 门禁不通过 = 禁止进入下一阶段 |
| **No Placeholders** | 禁止 TBD、模糊描述、占位符 |
| **RED-GREEN-REFACTOR** | 先写测试 → 运行失败 → 实现 → 通过 → 重构 |
| **任务粒度** | 每个任务 2-5 分钟 |
| **隔离执行** | Git Worktree 隔离 + Subagent 分发 |
| **1% Rule** | 有 1% 概率适用某 skill → 强制调用 |

---

## 2. Phase 1: Design (brainstorming)

### 2.1 目标

交互式设计，产出完整的设计规格文档。

### 2.2 触发命令

```text
主命令:
─────────────────────────────────────────
/superpowers              # 主入口，自动加载 brainstorming
/brainstorming            # 直接触发设计 skill

触发时机:
─────────────────────────────────────────
- 开始任何创造性工作
- 创建新功能、构建组件、添加功能
- "帮我设计"、"规划这个功能"
```

### 2.3 执行流程

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Phase 1: /superpowers 命令执行流程                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  用户: /superpowers                                                          │
│       ↓                                                                     │
│  Step 1: 探索项目上下文 (文件、文档、最近提交)                                 │
│       ↓                                                                     │
│  Step 2: 提供 UI/UX 视觉辅助 (如适用)                                        │
│       ↓                                                                     │
│  Step 3: 逐一提出澄清问题                                                    │
│       ↓                                                                     │
│  Step 4: 提出 2-3 个方案并说明取舍                                            │
│       ↓                                                                     │
│  Step 5: 分段呈现设计内容请求批准                                             │
│       ↓                                                                     │
│  Step 6: 写设计文档到 docs/superpowers/specs/                                │
│       ↓                                                                     │
│  Step 7: Self-Review (完整性/一致性/YAGNI)                                   │
│       ↓                                                                     │
│  Step 8: 输出设计规格文档                                                    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 2.4 各步骤角色分工

#### Step 1: 探索项目上下文

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 阅读 PRD/Design 文件，理解业务需求 | **主产出**: 需求理解 |
| **Architect** | 查看最近 git log，了解技术现状 | **主产出**: 技术背景 |
| **Architect** | 检查现有架构文档 Architect/*.md | **主产出**: 架构依赖分析 |
| **Platform Spec** | 查看现有代码实现，了解平台差异 | 辅助: 平台现状说明 |
| **QA** | 查看现有测试覆盖率，了解测试现状 | 辅助: 测试现状说明 |

#### Step 2: 提供 UI/UX 视觉辅助

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 展示 Design 设计稿截图或 mockup | **主产出**: UI 参考图 |
| **Architect** | 说明 UI 组件层级和交互流程 | **主产出**: UI 流程图 |
| **Platform Spec** | 提出平台特定 UI 规范建议 | 辅助: 平台 UI 建议 |

#### Step 3: 逐一提出澄清问题

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 提出技术可行性问题 | **主产出**: 问题清单 |
| **Architect** | 提出架构扩展性问题 | **主产出**: 问题清单 |
| **Platform Spec** | 提出平台兼容性问题 | 辅助: 补充问题 |
| **QA** | 提出测试覆盖边界问题 | 辅助: 补充问题 |

#### Step 4: 提出 2-3 个方案

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 设计 2-3 个技术方案 | **主产出**: 方案对比表 |
| **Architect** | 分析各方案优劣和取舍 | **主产出**: 方案分析 |
| **Platform Spec** | 补充平台实现差异说明 | 辅助: 平台差异备注 |
| **QA** | 补充测试难度评估 | 辅助: 测试难度说明 |

#### Step 5: 分段呈现设计内容

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 分段展示设计规格内容 | **主产出**: 设计段落 |
| **Architect** | 等待人工确认或修改 | **主产出**: 确认记录 |
| **Platform Spec** | 提出平台实现细节补充 | 辅助: 补充内容 |
| **QA** | 提出测试策略补充 | 辅助: 补充内容 |

#### Step 6: 写设计文档

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 撰写设计规格文档 | **主产出**: specs/*.md |
| **Architect** | 定义数据模型和 API | **主产出**: Models/API 定义 |
| **Platform Spec** | 补充平台特定实现说明 | 辅助: 平台说明章节 |
| **QA** | 补充测试策略章节 | 辅助: 测试策略章节 |

#### Step 7: Self-Review

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 检查完整性 (无 TBD) | **主产出**: 完整性确认 |
| **Architect** | 检查一致性 (无矛盾) | **主产出**: 一致性确认 |
| **Architect** | 检查 YAGNI (无过度设计) | **主产出**: YAGNI 确认 |
| **Platform Spec** | 检查平台可行性 | 辅助: 平台可行性确认 |
| **QA** | 检查测试可覆盖性 | 辅助: 测试覆盖确认 |

#### Step 8: 输出设计规格文档

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 最终文档输出并签字批准 | **主产出**: specs/*.md + HARD-GATE 1 通过签字 |
| **全员** | 确认文档内容无异议 | 辅助: 确认记录 |

### 2.5 输出

```
docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md
```

### 2.6 HARD-GATE 1

```
⚠️ HARD-GATE: 禁止在未批准设计前实施

未通过此门禁 → 禁止进入 Phase 2
禁止调用 /writing-plans 命令
```

### 2.7 Checklist

```text
Phase 1 Checklist:
─────────────────────────────────────────
☐ 探索项目上下文 (Architect + Platform Spec + QA)
☐ 提供 UI/UX 视觉辅助 (Architect)
☐ 逐一提出澄清问题 (全员)
☐ 提出 2-3 个方案 (Architect + Platform Spec + QA)
☐ 分段呈现设计 (Architect)
☐ 写设计文档 (Architect)
☐ 规格审查循环 (全员)
☐ HARD-GATE 1 签字 (Architect)
```

---

## 3. Phase 2: Planning (writing-plans)

### 3.1 目标

将设计规格分解为可执行的实施计划。

### 3.2 触发命令

```text
主命令:
─────────────────────────────────────────
/writing-plans            # 直接触发规划 skill

触发时机:
─────────────────────────────────────────
- Phase 1 HARD-GATE 1 通过后
- 设计文档已批准签字
```

### 3.3 执行流程

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Phase 2: /writing-plans 命令执行流程                        │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  用户: /writing-plans                                                        │
│       ↓                                                                     │
│  Step 1: 加载已批准设计规格                                                   │
│       ↓                                                                     │
│  Step 2: 文件结构映射 (新增/修改文件列表)                                      │
│       ↓                                                                     │
│  Step 3: 任务分解 (每任务 2-5 分钟)                                           │
│       ↓                                                                     │
│  Step 4: RED-GREEN-REFACTOR 循环定义                                         │
│       ↓                                                                     │
│  Step 5: No Placeholders 自动检查                                            │
│       ↓                                                                     │
│  Step 6: Self-Review (可构建性/规格对齐)                                      │
│       ↓                                                                     │
│  Step 7: 输出实施计划文档                                                     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 3.4 各步骤角色分工

#### Step 1: 加载已批准设计规格

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 加载 specs/*.md 文档 | **主产出**: 设计规格加载 |
| **Architect** | 向全员介绍设计重点 | **主产出**: 设计要点说明 |
| **Platform Spec** | 确认平台实现范围 | 辅助: 范围确认 |
| **QA** | 确认测试范围 | 辅助: 范围确认 |

#### Step 2: 文件结构映射

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 规划整体文件结构 | **主产出**: 文件结构列表 |
| **Architect** | 定义新增/修改文件 | **主产出**: 文件变更清单 |
| **iOS-Spec** | 规划 iOS 文件结构 | 辅助: iOS 文件建议 |
| **Android-Spec** | 规划 Android 文件结构 | 辅助: Android 文件建议 |
| **Harmony-Spec** | 规划 Harmony 文件结构 | 辅助: Harmony 文件建议 |
| **QA** | 规划测试文件结构 | 辅助: 测试文件建议 |

#### Step 3: 任务分解

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 分解整体任务为 2-5 分钟粒度 | **主产出**: 任务清单 |
| **Architect** | 定义任务依赖关系 | **主产出**: 依赖图 |
| **iOS-Spec** | 分解 iOS 平台任务 | 辅助: iOS 任务建议 |
| **Android-Spec** | 分解 Android 平台任务 | 辅助: Android 任务建议 |
| **Harmony-Spec** | 分解 Harmony 平台任务 | 辅助: Harmony 任务建议 |
| **QA** | 分解测试编写任务 | 辅助: 测试任务建议 |

#### Step 4: RED-GREEN-REFACTOR 定义

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 定义 TDD 循环步骤 | **主产出**: TDD 步骤定义 |
| **Platform Spec** | 定义平台特定测试策略 | 辅助: 平台测试建议 |
| **QA** | 定义测试验收标准 | 辅助: 验收标准建议 |

#### Step 5: No Placeholders 检查

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 自动检查 TBD/模糊描述 | **主产出**: 检查结果 |
| **Architect** | 人工确认无占位符 | **主产出**: 确认签字 |
| **全员** | 确认任务描述明确可执行 | 辅助: 确认记录 |

#### Step 6: Self-Review

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 检查可构建性 | **主产出**: 可构建性确认 |
| **Architect** | 检查规格对齐 | **主产出**: 规格对齐确认 |
| **Platform Spec** | 检查任务粒度合理性 | 辅助: 粒度确认 |
| **QA** | 检查测试覆盖完整性 | 辅助: 测试覆盖确认 |

#### Step 7: 输出实施计划文档

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 最终计划文档输出并签字批准 | **主产出**: plans/*.md + 批准签字 |
| **全员** | 确认计划可执行无异议 | 辅助: 确认记录 |

### 3.5 输入/输出

```text
输入: docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md (已批准)
输出: docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md
```

### 3.6 HARD-GATE 2

```
⚠️ HARD-GATE: 计划必须可构建

未通过此门禁 → 禁止进入 Phase 3
禁止调用 /subagent-driven-development 命令
```

### 3.7 No Placeholders 规则

```text
计划失败条件 (必须重新规划):
─────────────────────────────────────────
❌ TBD 字样
❌ 模糊描述 ("待确认"、"待定")
❌ 未定义引用 ("类似 Task N")
❌ 占位符 ("// ... existing code ...")
```

### 3.8 Checklist

```text
Phase 2 Checklist:
─────────────────────────────────────────
☐ 加载设计规格 (Architect)
☐ 文件结构映射 (Architect + Platform Spec + QA)
☐ 任务分解 2-5 分钟 (各角色分工)
☐ RED-GREEN-REFACTOR 定义 (Architect + QA)
☐ No Placeholders 检查 (Architect)
☐ 可构建性验证 (Architect + Platform Spec)
☐ 规格对齐验证 (Architect)
☐ HARD-GATE 2 签字 (Architect)
```

---

## 4. Phase 3: Implementation (SDD)

### 4.1 目标

在隔离环境中执行实施计划。

### 4.2 触发命令

```text
主命令:
─────────────────────────────────────────
/subagent-driven-development  # SDD 执行 (推荐)
/tdd                         # TDD 循环
/debug                       # 调试 (如遇问题)
/verify                      # 完成前验证

触发时机:
─────────────────────────────────────────
- Phase 2 HARD-GATE 2 通过后
- 开始实施代码
```

### 4.3 执行流程

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Phase 3: 多命令协同执行流程                                 │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  用户: 继续 Phase 3 实施                                                      │
│       ↓                                                                     │
│  Step 1: Git Worktree 隔离                                                   │
│          EnterWorktree({ name: "feature-001" })                             │
│       ↓                                                                     │
│  Step 2: /subagent-driven-development                                       │
│          Controller Agent 分发任务                                           │
│       ↓                                                                     │
│  ┌────┴────┬────────────┬────────────┐                                     │
│  │         │            │            │                                      │
│  ↓         ↓            ↓            ↓                                      │
│  iOS-Spec  Android-Spec Harmony-Spec QA                                     │
│  执行任务   执行任务       执行任务     编写测试                                │
│  RED-GREEN-REFACTOR                                                     │
│       ↓                                                                     │
│  Step 3: (如遇问题) /debug                                                   │
│          4阶段调试流程                                                        │
│       ↓                                                                     │
│  Step 4: /verify 完成前验证                                                  │
│       ↓                                                                     │
│  Step 5: ExitWorktree                                                       │
│       ↓                                                                     │
│  Step 6: 双阶段审查                                                          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 4.4 各步骤角色分工

#### Step 1: Git Worktree 隔离

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 创建 Worktree 隔离环境 | **主产出**: Worktree 目录 |
| **Architect** | 分配任务给各平台专家 | **主产出**: 任务分配表 |
| **iOS-Spec** | 进入 iOS worktree 目录 | 辅助: iOS 工作环境准备 |
| **Android-Spec** | 进入 Android worktree 目录 | 辅助: Android 工作环境准备 |
| **Harmony-Spec** | 进入 Harmony worktree 目录 | 辅助: Harmony 工作环境准备 |

#### Step 2: /subagent-driven-development 执行

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 监控任务执行进度 | 辅助: 进度监控 |
| **Architect** | 协调跨平台依赖 | 辅助: 协调记录 |
| **iOS-Spec** | 执行 iOS 任务: Models → ViewModel → View | **主产出**: iOS 源代码 |
| **Android-Spec** | 执行 Android 任务: Models → ViewModel → View | **主产出**: Android 源代码 |
| **Harmony-Spec** | 执行 Harmony 任务: Models → ViewModel → View | **主产出**: Harmony 源代码 |
| **QA** | 执行测试任务: 编写测试用例 | **主产出**: 测试源代码 |

#### Step 3: RED-GREEN-REFACTOR 循环 (每个任务)

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Platform Spec** | [RED] 写测试文件，运行确认失败 | **主产出**: 测试文件 |
| **Platform Spec** | [GREEN] 写实现代码，运行确认通过 | **主产出**: 实现文件 |
| **Platform Spec** | [REFACTOR] 优化代码质量 | **主产出**: 优化代码 |
| **QA** | 验证测试覆盖率和质量 | 辅助: 覆盖率确认 |
| **Platform Spec** | [COMMIT] git commit 提交代码 | **主产出**: 提交记录 |

#### Step 4: /debug 调试 (如遇问题)

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Platform Spec** | 描述问题现象 | **主产出**: 问题描述 |
| **Platform Spec** | 查看错误日志和堆栈 | **主产出**: 错误日志分析 |
| **Architect** | 协助分析根因 | 辅助: 根因分析建议 |
| **Platform Spec** | 制定修复方案并实施 | **主产出**: 修复代码 |
| **QA** | 验证修复后测试通过 | 辅助: 验证确认 |

#### Step 5: /verify 完成前验证

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **QA** | 运行所有测试 | **主产出**: 测试报告 |
| **QA** | 检查覆盖率 ≥80% | **主产出**: 覆盖率报告 |
| **Platform Spec** | 启动应用手动测试 | 辅助: 手测确认 |
| **Platform Spec** | 确认无编译错误/警告 | 辅助: 编译确认 |
| **Architect** | 确认所有任务已完成 | 辅助: 完成确认 |

#### Step 6: 双阶段审查

**Stage 1: Spec Compliance (Architect 主导)**

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 对照设计规格检查需求满足 | **主产出**: Spec 合规确认 |
| **Architect** | 检查数据模型一致性 | **主产出**: 数据模型确认 |
| **Architect** | 检查 API 接口一致性 | **主产出**: API 确认 |
| **Architect** | 检查 UI/UX 实现一致性 | **主产出**: UI 确认 |

**Stage 2: Code Quality (QA + Architect)**

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **QA** | 检查代码规范 | **主产出**: 规范确认 |
| **QA** | 验证测试覆盖率 | **主产出**: 覆盖率确认 |
| **Architect** | 检查安全漏洞 | **主产出**: 安全确认 |
| **QA** | 检查性能无回归 | **主产出**: 性能确认 |
| **Architect** | 最终批准签字 | **主产出**: HARD-GATE 3 通过签字 |

### 4.5 HARD-GATE 3

```
⚠️ HARD-GATE: 规格合规 + 代码质量

未通过此门禁 → 禁止合并到主分支
禁止调用 git merge / git push
```

### 4.6 Checklist

```text
Phase 3 Checklist:
─────────────────────────────────────────
☐ Git Worktree 隔离 (Architect)
☐ 任务分配 (Architect → Platform Spec + QA)
☐ /tdd RED-GREEN-REFACTOR (Platform Spec)
☐ (如遇问题) /debug (Platform Spec + Architect)
☐ /verify 完成验证 (QA + Platform Spec + Architect)
☐ Spec Compliance 检查 (Architect)
☐ Code Quality 检查 (QA + Architect)
☐ HARD-GATE 3 签字 (Architect + QA)
☐ 合并发布 (Architect)
```

---

## 5. 合并发布

### 5.1 各角色操作

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | git checkout main + merge | **主产出**: 合并记录 |
| **Architect** | git push + 创建 tag | **主产出**: 发布记录 |
| **Architect** | 更新 CHANGELOG.md | **主产出**: Changelog |
| **Platform Spec** | 更新模块记录 overview.md | 辅助: 模块记录更新 |
| **QA** | 更新测试报告归档 | 辅助: 测试归档 |

---

## 6. 命令速查表

| 阶段 | 命令 | 主导角色 |
|------|------|----------|
| **Phase 1** | `/superpowers`, `/brainstorming` | Architect |
| **Phase 2** | `/writing-plans` | Architect |
| **Phase 3** | `/subagent-driven-development` | Platform Spec |
| | `/tdd` | Platform Spec |
| | `/debug` | Platform Spec |
| | `/verify` | QA + Architect |

---

## 7. 相关文档

| 文档 | 路径 | 说明 |
|------|------|------|
| 团队角色 | [01-roles.md](./01-roles.md) | 角色职责定义 |
| 门禁详细定义 | [03-gates.md](./03-gates.md) | HARD-GATE 检查清单 |
| 开发规范 | [04-standards.md](./04-standards.md) | 测试 + Commit 规范 |
| Worktree 规范 | [05-worktree.md](./05-worktree.md) | 隔离环境使用 |
| 人工操作指南 | [06-operations.md](./06-operations.md) | 详细操作步骤 |

---

## 8. 其他开发场景

除新功能开发外，还有以下场景需使用不同流程：

### 8.1 场景分类

| 场景 | 说明 | 适用流程 |
|------|------|----------|
| **Bug 修复** | 修复已知 Bug | Bug 修复流程 (简化) |
| **旧功能改版** | 重构现有功能、改版 UI | 改版流程 (Phase 1-3 简化版) |
| **小改动/优化** | 文案修改、UI 微调、性能优化 | 小改动流程 (Phase 3 直接执行) |

---

### 8.2 Bug 修复流程

#### 适用条件

```text
触发条件:
─────────────────────────────────────────
- 已有 Bug 报告或 Issue
- 功能已存在，需修复问题
- 不涉及新功能设计
```

#### 流程图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Bug 修复流程 (简化版)                                       │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Step 1: Bug 确认                                                            │
│  /debug Define                                                              │
│       ↓                                                                     │
│  Step 2: 根因分析                                                            │
│  /debug Gather + Analyze                                                    │
│       ↓                                                                     │
│  Step 3: 修复方案                                                            │
│  /debug Fix                                                                 │
│       ↓                                                                     │
│  Step 4: 验证修复                                                            │
│  /verify                                                                    │
│       ↓                                                                     │
│  HARD-GATE 3: 修复验证通过                                                   │
│       ↓                                                                     │
│  合并发布                                                                    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### 角色分工

| 步骤 | 角色 | 操作 | 主产出/辅助 |
|------|------|------|-------------|
| **Bug 确认** | QA | 描述 Bug 现象和复现步骤 | **主产出**: Bug 描述 |
| | Platform Spec | 确认 Bug 是否存在 | 辅助: 确认记录 |
| **根因分析** | Platform Spec | 查看错误日志和堆栈 | **主产出**: 错误日志分析 |
| | Architect | 协助分析根因 | 辅助: 根因建议 |
| **修复方案** | Platform Spec | 制定修复方案 | **主产出**: 修复方案 |
| | Architect | 审核修复方案 | 辅助: 方案审核 |
| **修复实施** | Platform Spec | 编写修复代码 + 测试 | **主产出**: 修复代码 |
| **验证修复** | QA | 验证 Bug 已修复 | **主产出**: 验证报告 |
| | QA | 检查无新增 Bug | 辅助: 回归确认 |

#### HARD-GATE 要求

```text
Bug 修复 HARD-GATE (简化版):
─────────────────────────────────────────
☐ Bug 已修复，原复现步骤不再触发
☐ 测试覆盖率 ≥80% (新增代码部分)
☐ 无新增 Bug (回归测试通过)
☐ QA 批准签字
☐ Architect 批准签字 (可选，重大 Bug 需要)
```

---

### 8.3 旧功能改版/重构流程

#### 适用条件

```text
触发条件:
─────────────────────────────────────────
- 现有功能需重构或改版
- UI 设计有变更
- 代码架构需优化
- 涉及较大改动 (影响范围 >10%)

不适合:
─────────────────────────────────────────
- 仅修改文案或颜色 (使用小改动流程)
- 仅修复 Bug (使用 Bug 修复流程)
```

#### 流程图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    旧功能改版流程 (简化版)                                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Phase 1: 改版评估      HARD-GATE 1'     Phase 2: 改版计划                   │
│  评估影响范围      ─────────────────→   分解改版任务                          │
│       ↓            "评估通过方可改版"        ↓                               │
│  改版方案文档                           HARD-GATE 2'                         │
│                                        "计划必须可行"                         │
│                                             ↓                               │
│  Phase 3: 改版实施  ─────────────→   HARD-GATE 3                            │
│  /tdd /verify           ↓             "功能不退化 + 代码质量"                │
│       ↓             Git Worktree                                            │
│  双阶段审查           隔离机制                                               │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### Phase 1: 改版评估 (简化版)

| 步骤 | 角色 | 操作 | 主产出/辅助 |
|------|------|------|-------------|
| **影响分析** | Architect | 分析改版影响范围 | **主产出**: 影响范围报告 |
| | Platform Spec | 分析代码依赖关系 | 辅助: 依赖分析 |
| **方案设计** | Architect | 设计改版方案 | **主产出**: 改版方案文档 |
| | Platform Spec | 提出平台实现建议 | 辅助: 平台建议 |
| **风险评估** | Architect | 评估风险和回退方案 | **主产出**: 风险评估 |
| | QA | 评估测试影响 | 辅助: 测试影响分析 |

#### HARD-GATE 1' (改版评估)

```text
☐ 影响范围已明确
☐ 改版方案已设计
☐ 风险已评估，回退方案已定义
☐ Architect 批准签字
```

#### Phase 2: 改版计划 (简化版)

| 步骤 | 角色 | 操作 | 主产出/辅助 |
|------|------|------|-------------|
| **任务分解** | Architect | 分解改版任务 | **主产出**: 改版任务清单 |
| | Platform Spec | 分解平台任务 | 辅助: 平台任务建议 |
| **测试计划** | QA | 定义回归测试范围 | **主产出**: 回归测试计划 |

#### HARD-GATE 2' (改版计划)

```text
☐ 任务已分解 (粒度 2-5 分钟)
☐ 回归测试范围已定义
☐ Architect 批准签字
```

#### Phase 3: 改版实施

与新功能开发 Phase 3 相同，但需增加：

```text
额外检查:
─────────────────────────────────────────
☐ 原功能不退化 (回归测试通过)
☐ 性能无下降
☐ 数据兼容性保持
```

---

### 8.4 小改动/优化流程

#### 适用条件

```text
触发条件:
─────────────────────────────────────────
- 文案修改
- 颜色/间距调整
- 小性能优化
- 配置参数调整
- 影响范围 <5%

不适合:
─────────────────────────────────────────
- 功能逻辑变更 (使用改版流程)
- UI 结构变更 (使用改版流程)
```

#### 流程图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    小改动流程 (直接执行)                                        │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Step 1: 确认改动范围                                                        │
│       ↓                                                                     │
│  Step 2: 直接实施                                                            │
│  /tdd (如需测试)                                                             │
│       ↓                                                                     │
│  Step 3: 验证改动                                                            │
│  /verify                                                                    │
│       ↓                                                                     │
│  Step 4: 提交合并                                                            │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### 角色分工

| 步骤 | 角色 | 操作 | 主产出/辅助 |
|------|------|------|-------------|
| **确认改动** | Platform Spec | 确认改动内容和范围 | **主产出**: 改动说明 |
| **实施改动** | Platform Spec | 编写改动代码 | **主产出**: 改动代码 |
| **验证改动** | Platform Spec | 自测改动效果 | 辅助: 自测确认 |
| | QA | 验证无副作用 | 辅助: 回归确认 |

#### 检查要求 (无 HARD-GATE)

```text
小改动检查:
─────────────────────────────────────────
☐ 改动已完成
☐ 原功能正常 (自测通过)
☐ 无编译错误/警告
☐ 代码已提交
```

---

### 8.5 场景选择指南

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    开发场景选择决策树                                           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  开始                                                                        │
│     │                                                                        │
│     ↓                                                                        │
│  是否是新功能? ──────── 是 ───→ 完整三阶段流程 (Phase 1-3)                    │
│     │                                                                        │
│     否                                                                       │
│     │                                                                        │
│     ↓                                                                        │
│  是否是 Bug? ──────── 是 ───→ Bug 修复流程 (/debug + /verify)                 │
│     │                                                                        │
│     否                                                                       │
│     │                                                                        │
│     ↓                                                                        │
│  改动范围 >10%? ──────── 是 ───→ 改版流程 (Phase 1-3 简化版)                   │
│     │                                                                        │
│     否                                                                       │
│     │                                                                        │
│     ↓                                                                        │
│  小改动流程 (直接执行)                                                        │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 8.6 场景命令速查

| 场景 | 命令 | 主导角色 |
|------|------|----------|
| **Bug 修复** | `/debug` → `/verify` | Platform Spec + QA |
| **旧功能改版** | `/superpowers` (简化版) → `/writing-plans` (简化版) → `/tdd` | Architect + Platform Spec |
| **小改动** | `/tdd` (可选) → `/verify` | Platform Spec |

---

### 8.7 旧项目接入规范流程

#### 适用条件

```text
触发条件:
─────────────────────────────────────────
- 项目已存在，但没有规范文档
- 需要继续开发或维护
- 需要保持代码风格一致性
- 不想一次性重构所有代码

目标:
─────────────────────────────────────────
- 保持现有代码风格一致性
- 渐进式引入规范
- 新代码遵循新规范，旧代码保持原风格
```

#### 流程图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    旧项目接入规范流程                                           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Phase 0: 规范探索      规范文档产出      后续开发                             │
│  分析现有代码      ─────────────────→   按规范继续                            │
│       ↓                  ↓                    ↓                            │
│  提取现有约定        规范文档               新代码遵循新规范                   │
│  定义兼容规范        CLAUDE.md              旧代码保持原风格                   │
│                                                                             │
│  渐进式规范化策略:                                                            │
│  ───────────────────────────────────────────────────────────────────────    │
│  优先级 1: 新代码 → 强制遵循新规范                                            │
│  优先级 2: 改版代码 → 逐步迁移到新规范                                         │
│  优先级 3: 旧代码 → 保持原风格，不强制重构                                      │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

#### Phase 0: 规范探索步骤

**Step 1: 代码风格探索**

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 分析现有代码目录结构 | **主产出**: 目录结构分析 |
| **Platform Spec** | 分析命名约定 (类、方法、变量) | **主产出**: 命名约定分析 |
| **Platform Spec** | 分析代码组织模式 (MVVM/MVC/etc) | 辅助: 架构模式分析 |
| **QA** | 分析现有测试模式和覆盖率 | 辅助: 测试现状分析 |

**Step 2: 现有约定提取**

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 提取核心设计模式 | **主产出**: 设计模式清单 |
| **Platform Spec** | 提取平台特定约定 | **主产出**: 平台约定清单 |
| **Architect** | 提取 API 设计风格 | **主产出**: API 风格定义 |
| **Architect** | 提取数据模型风格 | **主产出**: 数据模型风格 |

**Step 3: 兼容规范定义**

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 定义新规范与旧规范的兼容点 | **主产出**: 兼容规范文档 |
| **Architect** | 定义渐进迁移策略 | **主产出**: 迁移策略 |
| **Platform Spec** | 确认平台兼容性 | 辅助: 平台兼容确认 |

**Step 4: 规范文档产出**

| 角色 | 操作 | 主产出/辅助 |
|------|------|-------------|
| **Architect** | 撰写项目规范文档 | **主产出**: CLAUDE.md (项目版) |
| **Architect** | 定义代码风格指南 | **主产出**: 风格指南章节 |
| **Platform Spec** | 补充平台特定规范 | 辅助: 平台规范补充 |

#### 产出物

```text
Docs/SOP/CLAUDE.md (项目版):
─────────────────────────────────────────
应包含:
- 现有代码风格分析结果
- 命名约定 (从现有代码提取)
- 目录结构约定
- 代码组织模式
- 新旧规范兼容策略
- 渐进迁移计划
```

#### 渐进式规范化策略

```text
三级优先级策略:
─────────────────────────────────────────

优先级 1: 新代码 (强制)
─────────────────────────────────────────
✅ 必须遵循新规范
✅ 使用新命名约定
✅ 使用新架构模式
✅ 写测试 (覆盖率 ≥80%)

优先级 2: 改版代码 (渐进)
─────────────────────────────────────────
⚠️ 逐步迁移到新规范
⚠️ 改版范围内使用新风格
⚠️ 边界处保持兼容
⚠️ 补充测试

优先级 3: 旧代码 (保持)
─────────────────────────────────────────
🔄 保持原有风格
🔄 不强制重构
🔄 仅在有 Bug 时修复
🔄 不增加技术债
```

#### 实际开发规则

```text
开发规则:
─────────────────────────────────────────

规则 1: 新代码 = 新规范
─────────────────────────────────────────
新文件:
- 命名: 遵循新规范 (如 CourseListViewModel)
- 架构: 遵循 MVVM
- 测试: 必须写测试

规则 2: 改版代码 = 渐进迁移
─────────────────────────────────────────
改版文件:
- 文件名: 保持原名，不强制改名
- 新增代码: 使用新规范
- 修改代码: 逐步迁移到新规范
- 边界: 保持与旧代码兼容

规则 3: 旧代码 = 保持原样
─────────────────────────────────────────
旧文件:
- 不主动重构
- 仅修复 Bug
- 保持原有命名和结构
```

#### 规范探索检查清单

```text
Phase 0 Checklist:
─────────────────────────────────────────
☐ 目录结构分析完成
☐ 命名约定提取完成
☐ 设计模式提取完成
☐ 平台约定提取完成
☐ API 风格定义完成
☐ 数据模型风格定义完成
☐ 兼容规范文档产出
☐ 渐进迁移策略定义
☐ CLAUDE.md (项目版) 创建
☐ HARD-GATE 0' 人工确认签字
```

#### HARD-GATE 0': 规范确认

```text
⚠️ HARD-GATE 0': 提取的规范必须人工确认

未通过此门禁 → 禁止使用提取的规范继续开发
禁止进入后续开发流程
```

#### 人工确认流程

| 步骤 | 角色 | 操作 | 主产出/辅助 |
|------|------|------|-------------|
| **Step 1: 规范展示** | Architect | 展示提取的规范内容 | **主产出**: 规范展示 |
| **Step 2: 逐一确认** | 全员 | 逐项确认规范是否准确 | 辅助: 确认记录 |
| **Step 3: 修改调整** | Architect | 根据反馈修改规范 | **主产出**: 修改后的规范 |
| **Step 4: 最终签字** | Architect + 全员 | 签字批准规范文档 | **主产出**: 签字批准 |

#### 人工确认内容

```text
需要确认的内容:
─────────────────────────────────────────

1. 目录结构确认
─────────────────────────────────────────
☐ 目录命名是否准确
☐ 目录层级是否合理
☐ 是否有遗漏的目录

2. 命名约定确认
─────────────────────────────────────────
☐ 类名命名约定是否准确
☐ 方法名命名约定是否准确
☐ 变量名命名约定是否准确
☐ 文件名命名约定是否准确

3. 设计模式确认
─────────────────────────────────────────
☐ 当前架构模式是否准确 (MVC/MVVM/etc)
☐ 是否有遗漏的设计模式
☐ 模式使用场景是否准确

4. 平台约定确认
─────────────────────────────────────────
☐ iOS 特定约定是否准确
☐ Android 特定约定是否准确
☐ Harmony 特定约定是否准确

5. API 风格确认
─────────────────────────────────────────
☐ API 命名风格是否准确
☐ API 参数风格是否准确
☐ API 返回风格是否准确

6. 数据模型确认
─────────────────────────────────────────
☐ 数据结构风格是否准确
☐ 数据命名风格是否准确
☐ 数据关系定义是否准确

7. 渐进迁移策略确认
─────────────────────────────────────────
☐ 新代码规范是否合理
☐ 改版迁移策略是否合理
☐ 旧代码保持策略是否合理
```

#### 签字批准模板

```markdown
---
**规范确认人:** @architect
**确认日期:** YYYY-MM-DD
**HARD-GATE 0':** ✅ 通过

**确认签字:**
- 目录结构: ✅ @全员
- 命名约定: ✅ @全员
- 设计模式: ✅ @全员
- 平台约定: ✅ @Platform-Spec
- API 风格: ✅ @architect
- 数据模型: ✅ @architect
- 渐进迁移策略: ✅ @architect

签字后可按提取的规范继续开发
```

#### 确认失败处理

```text
确认失败处理:
─────────────────────────────────────────
Step 1: 记录问题点
─────────────────────────────────────────
Architect 记录每项确认失败的问题

Step 2: 重新提取
─────────────────────────────────────────
Architect 根据反馈重新提取规范

Step 3: 再次展示确认
─────────────────────────────────────────
重复确认流程直到全部通过

Step 4: 最终签字
─────────────────────────────────────────
全员签字批准后生效
```

#### 示例：从现有代码提取规范

**命名约定提取示例:**

```text
观察现有代码:
─────────────────────────────────────────
文件: OldCourseListViewController.swift
类名: CourseListViewController (旧风格: ViewController)
方法: loadCourses(), showCourseDetail() (旧风格: 无前缀)
变量: courseList, selectedCourse (旧风格: 驼峰式)

提取约定:
─────────────────────────────────────────
旧规范:
- 类名: {功能}ViewController
- 方法: {动作}()
- 变量: 驼峰式，无前缀

新规范 (兼容):
- 新文件: {功能}ViewModel / {功能}View (MVVM)
- 方法: 保持动词风格
- 变量: 驼峰式，保持一致
```

**目录结构提取示例:**

```text
观察现有目录:
─────────────────────────────────────────
项目/
├── Controllers/     (旧风格: MVC)
├── Models/
├── Views/
├── Services/
└── Utils/

新目录兼容:
─────────────────────────────────────────
项目/
├── Controllers/     (旧代码保持)
├── ViewModels/      (新代码使用)
├── Models/          (共用)
├── Views/           (共用，逐步迁移)
├── Services/        (共用)
└── Utils/           (共用)
```

---

**版本**: 3.5
**更新日期**: 2026-04-14
**适用范围**: 跨平台移动开发团队
**来源**: Superpowers Pipeline v6.1